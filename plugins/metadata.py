...
async def handle_metadata(bot: Client, message: Message):
    ...
    ms = await message.reply_text("**⏳ Hold on, sweetheart... I’m getting things ready for you~**", reply_to_message_id=message.id)
    ...
    f"**Oooh~ Check out your current metadata, cutie! 😘**\n\n➜ `{user_metadata}`",
    quote=True,
    reply_markup=InlineKeyboardMarkup(ON)
)
    return await message.reply_text(
    f"**Hey babe, here’s your current metadata just for you! Wanna make it even sexier? 😉**\n\n➜ `{user_metadata}`",
    quote=True,
...
async def query_metadata(bot: Client, query: CallbackQuery):
    ...
    f"**Hey cutie, here’s your current metadata just for you! Wanna spice it up? 😉**\n\n➜ `{user_metadata}`",
    reply_markup=InlineKeyboardMarkup(OFF)
)
            ...
            await query.message.edit(
    f"**Hey there, sexy! Here’s your current metadata, looking all fine and ready for a makeover! 😘**\n\n➜ `{user_metadata}`",
    reply_markup=InlineKeyboardMarkup(ON)
)
