...
async def rename_start(client, message):
    ...
    if ban_chk:
        return await message.reply("**You are banned from using this bot. Please contact @Suh0_Kang to resolve the issue!**")

    if file.file_size > 2000 * 1024 * 1024:
        return await message.reply_text("Oops~ That file’s too big to handle right now… Maybe try a smaller one?")
    ...
    try:
        ...
        await sleep(e.value)
        await message.reply_text(
            text=f"**Hey cutie, enter the new filename for me~**\n\n**Old File Name** :- `{filename}`",
            reply_to_message_id=message.id,
            reply_markup=ForceReply(selective=True)
        )
...
async def refunc(client, message):
    ...
    if (reply_message.reply_markup) and isinstance(reply_message.reply_markup, ForceReply):
        ...
        if not file:
            return await message.reply("Original file not found.")
...
async def doc(bot, update):
    ...
    try:
        ...
        return await update.message.edit(f"Can't set prefix/suffix.\n\n**Error:** `{e}`")
    ...
    try:
        path = await bot.download_media(
            ...
            progress_args=("🚀 Downloading in progress...", ms, time.time())
        )
    except Exception as e:
    ...
    if c_caption:
        try:
            ...
            return await ms.edit(text=f"Caption formatting error: ({e})")
    else:
    ...
    if (media.thumbs or c_thumb):
        try:
            if c_thumb:
                ...
                ph_path_ = await take_screen_shot(file_path, os.path.dirname(file_path), random.randint(0, max(1, duration - 1)))
    ...
    try:
        if update.message.text != "💠 Uploading...":
    ...
    try:
        ...
        deletion_msg = await sent_message.reply(
            text="**🗑 This file will auto-delete after 30 minutes. Save it soon!**",
        )
